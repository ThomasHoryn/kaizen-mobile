...
import com.kaizenmobile.service.AppUserQueryService;
...
/**
 * REST controller for managing {@link com.kaizenmobile.domain.AppUser}.
 */
@RestController
...
public class AppUserResource {
    ...
    public ResponseEntity<AppUser> createAppUser(@Valid @RequestBody AppUser appUser) throws URISyntaxException {
        ...
        if (Objects.isNull(appUser.getInternalUser())) {
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<AppUser>> getAllAppUsers(AppUserCriteria criteria) {
        log.debug("REST request to get AppUsers by criteria: {}", criteria);

        List<AppUser> entityList = appUserQueryService.findByCriteria(criteria);
